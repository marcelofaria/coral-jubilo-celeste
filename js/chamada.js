...
function renderHistory() { const dates = Object.keys(state.attendances).sort().reverse(); $('#history-list').innerHTML = dates.length ? dates.map(date => { const records = Object.values(state.attendances[date]); return `<button class="history-item ${date === state.activeDate ? 'active' : ''}" data-date="${date}"><strong>${formatDate(date)}</strong><span>${records.filter(status => status === 'present').length} presentes · ${records.filter(status => status === 'absent').length} ausências</span></button>` }).join('') : '<p class="history-hint">A primeira chamada aparecerá aqui ao marcar alguém.</p>' }
...
$('#new-attendance').addEventListener('click', () => { state.activeDate = today(); ensureAttendance(); save('nova chamada pronta'); render() })
...
$('#history-list').addEventListener('click', event => { const item = event.target.closest('[data-date]'); if (item) { state.activeDate = item.dataset.date; save(); render() } })
...
$('#export-data').addEventListener('click', () => { const blob = new Blob([JSON.stringify(state, null, 2)], { type: 'application/json' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `chamada-coral-${today()}.json`; link.click(); URL.revokeObjectURL(link.href) })
...
$('#import-data').addEventListener('change', event => { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const imported = JSON.parse(reader.result); if (!Array.isArray(imported.members) || typeof imported.attendances !== 'object') throw new Error(); state = { members: sortMembers(imported.members), attendances: imported.attendances, activeDate: imported.activeDate || today() }; save('backup importado'); render() } catch { window.alert('Não foi possível ler este backup.') } }; reader.readAsText(file); event.target.value = '' })
